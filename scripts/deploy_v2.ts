...
async function main() {
  ...
  // console.log(`Verify contract by command: npx hardhat verify ${nftMarketplace.address} ${NFT_ADDRESS} ${CURRENCY_TOKEN_ADDRESS} ${PLATFORM_TREASURY_ADDRESS} ${TRADING_FEE_ADDRESS} ${feePercentage} ${fixedFee} ${minimumTradePrice} ${ADMIN_ADDRESS} --network ${hre.network.name}`);
}
